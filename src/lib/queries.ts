...
export async function brandsOf(programId: number): Promise<{ id: number; name: string; slug: string; logo: string | null; n: number }[]> {
  ...
  const res = await db.drizzle.execute(
    sql`select b.id, b.name, b.slug, b.logo_url as logo, count(h.id)::int as n from brands b left join hotels h on h.brand_id = b.id and h._status = 'published' where b.program_id = ${programId} group by b.id, b.name, b.slug, b.logo_url order by n desc, b.name`,
  )
